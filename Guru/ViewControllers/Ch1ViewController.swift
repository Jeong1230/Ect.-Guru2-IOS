...
class Ch1ViewController: UIViewController {
    ...
    @IBAction func saveChallenge(_ sender: Any) {
        ...
        if let data = image.pngData() {
            ...
            StorageManager.shared.uploadImage(with: data, path: pathCh1, fileName: file_name) { result in
                switch result {
                case .success(let downloadUrl):
                    ...
                    self.db.collection("Challenge1").addDocument(data: [
                    ...
                    // 스토리보드 이동.
                    if let vc = self.storyboard?.instantiateViewController(withIdentifier: "ch1Done")as? Ch1DoneViewController {
...
extension Ch1ViewController:UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    // 갤러리 열기.
    func showGallery(){
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        if let url = info[.imageURL] as? URL {
            file_name = (url.lastPathComponent as NSString).deletingPathExtension + "_CH01.png"
