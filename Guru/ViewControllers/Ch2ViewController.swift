...
class Ch2ViewController: UIViewController {
    ...
    @IBAction func saveChallenge(_ sender: Any) {
        ...
        if let data = image.pngData() {
            ...
            StorageManager.shared.uploadImage(with: data, path: pathCh2, fileName: file_name) { result in
                switch result {
                case .success(let downloadUrl):
                    ...
                    self.db.collection("Challenge2").addDocument(data: [
                    ...
                    // 스토리보드 이동.
                    if let vc = self.storyboard?.instantiateViewController(withIdentifier: "ch2Done")as? Ch2DoneViewController {
...
extension Ch2ViewController:UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        if let url = info[.imageURL] as? URL {
            file_name = (url.lastPathComponent as NSString).deletingPathExtension + "_CH02.png"
